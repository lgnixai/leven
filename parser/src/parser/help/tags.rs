...
use tsr_lexer::globals::{Input, new_input, PineResult};
...
pub fn ok_tag(input: Input<'_>) -> PineResult<Positioned<Input<'_>>> {
    // 匹配 "ok"，并转换为 Positioned 类型
    //let parser = map(tag("ok"), |s: Input<'_>| to_positioned(s));
    ...
    // verify(
    //     map(tag("ok"), |s: Input<'_>| s.into()), // 转换为 Positioned
    //     |pos: &Positioned<Input<'_>>| pos.value.fragment() == "ok" // 验证内容
    // )(input)
    // verify(map(tag("ok"), |s: Input|s.into() ), |t| {
    //
    //
    //
    //
    ...
    // let parser = map(tag("ok"), |s: Input<'_>| Positioned::new(s));
    //
    // // 运行解析器
    // let result = parser(input)?;
    //
    // // 验证 "ok" 是否在 input 中
    // if result.value.fragment() == "ok" {
    //     Ok((result.value, result.value)) // 返回解析的结果
    // } else {
    //     Err(nom::Err::Failure(result)) // 解析失败
    // }
}